...
docker_package:
	...
	@echo '==========================================================================='
	docker build . -t kuchedav/llm:$(PACKAGE_VERSION_CLEAN)

docker_hub_push: docker_package
	docker push kuchedav/llm:$(PACKAGE_VERSION_CLEAN)

docker_clean:
	docker rmi kuchedav/llm:$(PACKAGE_VERSION_CLEAN)

helm: docker_hub_push docker_clean
	sed -i "" "/^\([[:space:]]*version: \).*/s//\1$(PACKAGE_VERSION_CLEAN)/" helm/Chart.yaml
	helm lint ./helm/
	helm uninstall llm
	helm install llm ./helm
